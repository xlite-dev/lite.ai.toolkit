...
void UNet::inference(std::vector<std::string> input, std::vector<std::vector<float>> &output) {
    ...
    for (auto t : timesteps)
    {
        ...
        // 根据当前时间步t生成文件名
        std::string filename = "/home/wangzijian/lite.ai.toolkit/output_image_" + std::to_string(static_cast<int>(t)) + ".png";
        save_output_as_image(latents, filename);
        save_to_bin(latents, "/home/wangzijian/lite.ai.toolkit/final_latent_data.bin");
        latents.insert(latents.end(), latents.begin(), latents.end());
...
//        size_t total_size = output_embedding.size() * output_embedding.size() +
//                            negative_output_embedding.size() * negative_output_embedding.size();
